...
class B_Instansi extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstinstansi('getAll', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstinstansi('getData', 'idInstansi', $value_data, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
                $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                if ($kolom == 'namaInstansi') {
                    $Query = "CALL sp_mstinstansi('getData', 'namaInstansi', NULL, NULL, '%$value_data%', NULL, NULL, NULL, NULL, NULL);";
                }
                $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            }
            if ($Fetch) {
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstinstansi('create', NULL, NULL, $dataParsed[idKota], '$dataParsed[namaInstansi]', '$dataParsed[alamatInstansi]', '$dataParsed[kepalaInstansi]', '$dataParsed[jbt_kplInstansi]', '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstinstansi('update', NULL, $dataParsed[idInstansi], $dataParsed[idKota], '$dataParsed[namaInstansi]', '$dataParsed[alamatInstansi]' , '$dataParsed[kepalaInstansi]', '$dataParsed[jbt_kplInstansi]', NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstinstansi('delete', NULL, $dataParsed[idInstansi], NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
