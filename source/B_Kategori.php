...
class B_Kategori extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_mstkategori('getAll', NULL, NULL, NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstkategori('getData', 'idKategori', $value_data, NULL, NULL, NULL);";
                $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                if ($kolom == 'namaKategori') {
                    $Query = "CALL sp_mstkategori('getData', 'namaKategori', NULL, '%$value_data%', NULL, NULL);";
                }
                $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            }
            if ($Fetch) {
    ...
    protected function post() {
        $this->app->post($this->pattern.'/', function(Request $request, Response $response) {
            ...
            $Query = "CALL sp_mstkategori('create', NULL, NULL, '$dataParsed[namaKategori]', '$dataParsed[createdBy]', NULL);";
    ...
    protected function put() {
        $this->app->put($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstkategori('update', NULL, $dataParsed[idKategori], '$dataParsed[namaKategori]', NULL, '$dataParsed[updatedBy]');";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstkategori('delete', NULL, $dataParsed[idKategori], NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
