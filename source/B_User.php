...
class B_User extends Library {
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_mstuser('getData', 'idUser', '$value_data', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
                $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                if ($kolom == 'idPerusahaan') {
                    $Query = "CALL sp_mstuser('getData', 'idPerusahaan', NULL, $value_data, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
                } if ($kolom !== 'idPerusahaan') {
                    $Query = "CALL sp_mstuser('getData', 'namaUser', NULL, NULL, NULL, NULL, NULL, '%$value_data%', NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
                }
                $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            }
            if (!empty($Fetch)) {
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstuser('delete', NULL, '$dataParsed[idUser]', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);";
            $Exec = $this->db->prepare($Query)->execute();
            if ($Exec) {
                return $response->withJson(["status" => "success"], 200);
            } else {
    ...
    public function login() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_mstuser('login', NULL, '$dataParsed[idUser]', NULL, NULL, NULL, NULL, NULL, '$dataParsed[passUser]', NULL, NULL, NULL, NULL, NULL, NULL);";
            $Fetch = $this->db->query($Query)->fetch(PDO::FETCH_OBJ);
            if ($Fetch) {
                return $response->withJson($Fetch, 200);
            } else {
