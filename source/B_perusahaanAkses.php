...
class B_perusahaanAkses extends Library {
    ...
    protected function getAll() {
        $this->app->get($this->pattern, function(Request $request, Response $response) {
            $Query = "CALL sp_utlperusahaanakses('getAll', NULL, NULL, NULL);";
    ...
    protected function getData() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            ...
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_utlperusahaanakses('getDataSite', 'idGrup', $value_data, NULL);";
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                $Query = "CALL sp_utlperusahaanakses('getDataSite', '$kolom', '%$value_data%', NULL);";
            }
            $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            if ($Fetch) {
                return $response->withJson($Fetch, 200);
    ...
    protected function getView() {
        $this->app->get($this->pattern.'/{VALUE_DATA}[/{KOLOM}]', function(Request $request, Response $response, $args) {
            $value_data = $args['VALUE_DATA'];
            if (empty($args['KOLOM'])) {
                $Query = "CALL sp_utlperusahaanakses('getViewSite', 'idGrup', $value_data, NULL);";
            } if (!empty($args['KOLOM'])) {
                $kolom = $args['KOLOM'];
                $Query = "CALL sp_utlperusahaanakses('getViewSite', '$kolom', '%$value_data%', NULL);";
            }
            $Fetch = $this->db->query($Query)->fetchAll(PDO::FETCH_OBJ);
            if ($Fetch) {
                return $response->withJson($Fetch, 200);
    ...
    protected function delete() {
        $this->app->delete($this->pattern.'/', function(Request $request, Response $response) {
            $dataParsed = $request->getParsedBody();
            $Query = "CALL sp_utlperusahaanakses('delete', NULL, $dataParsed[idGrup], NULL);";
